...
SUBJECT_NUM_DEEPCAM1 = 2288
SUBJECT_NUM_DEEPCAM2 = 1046
SUBJECT_NUM_DEEPCAM3 = 7689
SUBJECT_NUM_FACEWH_JIEPAI = 470
SUBJECT_NUM_MEGAFACE = 2209
SUBJECT_NUM_XIANNING = 0
SUBJECT_NUM_MSCELEB_1M = 74649
SUBJECT_NUM_WUHAN = 7028
SUBJECT_NUM_31239_PAI3PI = 2714
SUBJECT_NUM_FACTORYONE = 1077
SUBJECT_NUM_FACTORYTWO = 726
...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.subject_num = SUBJECT_NUM_VGG2 + SUBJECT_NUM_DEEPCAM1 + SUBJECT_NUM_DEEPCAM2 + SUBJECT_NUM_DEEPCAM3 + SUBJECT_NUM_FACEWH_JIEPAI + \
                           SUBJECT_NUM_MEGAFACE + SUBJECT_NUM_XIANNING + SUBJECT_NUM_WUHAN + SUBJECT_NUM_31239_PAI3PI+ \
                           SUBJECT_NUM_FACTORYONE + SUBJECT_NUM_FACTORYTWO
        ...
        for gpu_id in range(self.num_gpus):
            with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_id)):
                ...
                self.D_R_id_logits, self.D_R_fx, self.D_R_ln_w = self.discriminator(self.images_splits[gpu_id], is_reuse=reuse)
                self.m_l2 = tf.nn.l2_loss(tf.matmul(self.D_R_fx, self.D_R_ln_w)) / self.subject_num / self.gfc_dim

                self.d_acc_i = slim.metrics.accuracy(tf.argmax(self.D_R_id_logits, 1), self.input_labels_splits[gpu_id], weights=100.0)
                ...
                grads = opt.compute_gradients(self.d_loss_real_id_i + self.m_l2)
    ...
    def train(self, config):
        ...
        if config.dataset == 'VGG2':
            ...
            init_id_l, init_id_n = get_right_init_id(pid_VGG2_l, pid_VGG2_n, 0, 0)
            path_DeepCam1_l, pid_DeepCam1_l, path_DeepCam1_n, pid_DeepCam1_n = load_DeepCam1_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_DeepCam1_l, pid_DeepCam1_n, init_id_l, init_id_n)
            path_DeepCam2_l, pid_DeepCam2_l, path_DeepCam2_n, pid_DeepCam2_n = load_DeepCam2_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_DeepCam2_l, pid_DeepCam2_n, init_id_l, init_id_n)
            path_DeepCam3_l, pid_DeepCam3_l, path_DeepCam3_n, pid_DeepCam3_n = load_DeepCam3_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_DeepCam3_l, pid_DeepCam3_n, init_id_l, init_id_n)
            path_FaceWH_JiePai_l, pid_FaceWH_JiePai_l, path_FaceWH_JiePai_n, pid_FaceWH_JiePai_n = load_FaceWH_JiePai_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)
            
            init_id_l, init_id_n = get_right_init_id(pid_FaceWH_JiePai_l, pid_FaceWH_JiePai_n, init_id_l, init_id_n)
            path_MegaFace_l, pid_MegaFace_l, path_MegaFace_n, pid_MegaFace_n = load_MegaFace_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_MegaFace_l, pid_MegaFace_n, init_id_l, init_id_n)
            path_Xianning_l, pid_Xianning_l, path_Xianning_n, pid_Xianning_n = load_Xianning_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            #init_id_l, init_id_n = get_right_init_id(pid_Xianning_l, pid_Xianning_n, init_id_l, init_id_n)
            #path_MSCeleb_1M_l, pid_MSCeleb_1M_l, path_MSCeleb_1M_n, pid_MSCeleb_1M_n = load_MSCeleb_1M_by_list(
            #    initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_Xianning_l, pid_Xianning_n, init_id_l, init_id_n)
            path_Wuhan_l, pid_Wuhan_l, path_Wuhan_n, pid_Wuhan_n = load_Wuhan_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_Wuhan_l, pid_Wuhan_n, init_id_l, init_id_n)
            path_31239_pai3pi_l, pid_31239_pai3pi_l, path_31239_pai3pi_n, pid_31239_pai3pi_n = load_31239_pai3pi_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            init_id_l, init_id_n = get_right_init_id(pid_31239_pai3pi_l, pid_31239_pai3pi_n, init_id_l, init_id_n)
            path_FactoryOne_l, pid_FactoryOne_l, path_FactoryOne_n, pid_FactoryOne_n = load_FactoryOne_by_list(
                initial_id_l=init_id_l, initial_id_n=init_id_n)

            #init_id_l, init_id_n = get_right_init_id(pid_FactoryOne_l, pid_FactoryOne_n, init_id_l, init_id_n)
            #path_FactoryTwo_l, pid_FactoryTwo_l, path_FactoryTwo_n, pid_FactoryTwo_n = load_FactoryTwo_by_list(
            #    initial_id_l=init_id_l, initial_id_n=init_id_n)

            path_all = path_VGG2_n + path_DeepCam1_n + path_DeepCam2_n + path_DeepCam3_n + path_FaceWH_JiePai_n + path_MegaFace_n + \
                       path_Xianning_n + path_Wuhan_n + path_31239_pai3pi_n + path_FactoryOne_n #+ path_FactoryTwo_n

            pid_all = pid_VGG2_n + pid_DeepCam1_n + pid_DeepCam2_n + pid_DeepCam3_n + pid_FaceWH_JiePai_n + pid_MegaFace_n + \
                      pid_Xianning_n + pid_Wuhan_n + pid_31239_pai3pi_n + pid_FactoryOne_n #+ pid_FactoryTwo_n
            ...
            '''print("Identity")
            print  [(len(np.unique(pid_VGG2_n))/len(np.unique(pid_all))),
                    (len(np.unique(pid_DeepCam1_n))/len(np.unique(pid_all))),
                    (len(np.unique(pid_DeepCam2_n)) / len(np.unique(pid_all))),
                    (len(np.unique(pid_DeepCam3_n)) / len(np.unique(pid_all))),
                    (len(np.unique(pid_FaceWH_JiePai_n)) / len(np.unique(pid_all))),
                    (len(np.unique(pid_MegaFace_n)) / len(np.unique(pid_all))),
                    (len(np.unique(pid_MSCeleb_1M_n)) / len(np.unique(pid_all))),
                    (len(np.unique(pid_Wuhan_n)) / len(np.unique(pid_all)))]
            print("Images")
            print  [len(path_VGG2_n)/len(path_all),
                    len(path_DeepCam1_n) / len(path_all),
                    len(path_DeepCam2_n) / len(path_all),
                    len(path_DeepCam3_n) / len(path_all),
                    len(path_FaceWH_JiePai_n) / len(path_all),
                    len(path_MegaFace_n) / len(path_all),
                    len(path_MSCeleb_1M_n) / len(path_all),
                    len(path_Wuhan_n) / len(path_all)]'''
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                    ...
                    if np.mod(counter, 25) == 1:
                        ...
                        self.sess.run(self.train_op, feed_dict=ffeed_dict)
